...
void main() {
  test('CLI subtasks init creates subtasks file for active task', () async {
    final temp = Directory.systemTemp.createTempSync('genaisys_subtasks_');
    addTearDown(() {
    ...
    await runner.run(['scaffold', 'subtasks', temp.path]);

    final subtasksPath =
        '${layout.taskSpecsDir}${Platform.pathSeparator}my-task-subtasks.md';
    expect(File(subtasksPath).existsSync(), isTrue);
